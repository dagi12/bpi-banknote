...
class Banknot():
    ...
    def xor(self):
        return "".join(
            [chr(ord(c1) ^ ord(c2)) for (c1, c2) in zip(str(self.__ciag_identyfikacyjny), str(self.__ciag_R))])
    ...
    def zakryj_banknot(self, bank):
        ...
        self.__M = str(self.kwota) + "|" + str(self.identyfikator) + "|" + str(self.ciag_T) + "|" + str(
            self.ciag_W) + "|" + str(self.ciag_S) + "|" + str(self.ciag_U)
